...
func InitDB(db apid.DB) error {
	_, err := db.Exec(`
	CREATE TABLE IF NOT EXISTS kms_deployments (
...
func insertDeployments(tx *sql.Tx, deps []DataDeployment) error {
	...
	log.Debugf("inserting %d kms_deployments", len(deps))

	stmt, err := tx.Prepare(`
	INSERT INTO kms_deployments
	...
	`)
	if err != nil {
		log.Errorf("prepare insert into kms_deployments failed: %v", err)
		return err
	}
	defer stmt.Close()
	...
	for _, dep := range deps {
		...
		if err != nil {
			log.Errorf("insert into kms_deployments %s failed: %v", dep.ID, err)
			return err
		}
	}

	log.Debug("inserting kms_deployments succeeded")
	return err
}
...
func deleteDeployment(tx *sql.Tx, depID string) error {
	...
	stmt, err := tx.Prepare("DELETE FROM kms_deployments where id = $1;")
	if err != nil {
		log.Errorf("prepare delete from kms_deployments %s failed: %v", depID, err)
		return err
	}
	defer stmt.Close()
	...
	if err != nil {
		log.Errorf("delete from kms_deployments %s failed: %v", depID, err)
		return err
	}
...
func getDeployments(where string, a ...interface{}) (deployments []DataDeployment, err error) {
	...
	if err != nil {
		...
		log.Errorf("Error querying kms_deployments: %v", err)
		return
	}
	defer rows.Close()
...
func setDeploymentResults(results apiDeploymentResults) error {
	...
	for _, result := range results {
		...
		if err != nil {
			log.Errorf("update kms_deployments %s to %s failed: %v", result.ID, result.Status, err)
			return err
		}
...
func updateLocalBundleURI(depID, localBundleUri string) error {
	...
	stmt, err := getDB().Prepare("UPDATE kms_deployments SET local_bundle_uri=$1 WHERE id=$2;")
	if err != nil {
	...
	if err != nil {
		log.Errorf("update kms_deployments %s localBundleUri to %s failed: %v", depID, localBundleUri, err)
		return err
	}

	log.Debugf("update kms_deployments %s localBundleUri to %s succeeded", depID, localBundleUri)
